...
class TestClassifier(TestCase):
    ...
    def test_performance(self):
        rfc = Classifier(sklEstimator=RandomForestClassifier())
        rfc.fit(sample=enmapClassificationSample)
        rfc.performanceCrossValidation(nfold=10).report().saveHTML(filename=join(outdir, 'ClassifierPerformanceCrossValidation.html'))
...
class TestLogger(TestCase):
    ...
    def test_setAndLogSklEstimators(self):
        ...
        LoggerFlowObject(filename=r'c:\test\logLinearRegression.txt').setSklEstimatorItems(estimator=estimator.sklEstimator()).logItems()

        from sklearn.ensemble import RandomForestRegressor
        estimator = Regressor(sklEstimator=RandomForestRegressor(oob_score=True)).fit(sample=enmapRegressionSample)
        LoggerFlowObject(filename=r'c:\test\logRandomForestRegressor.txt').setSklEstimatorItems(estimator=estimator.sklEstimator()).logItems()

        from sklearn.cross_decomposition import PLSRegression
        estimator = Regressor(sklEstimator=PLSRegression(n_components=3, scale=True)).fit(sample=enmapRegressionSample)
        LoggerFlowObject(filename=r'c:\test\logPLSRegression.txt').setSklEstimatorItems(estimator=estimator.sklEstimator()).logItems()

        from sklearn.ensemble import RandomForestClassifier
        estimator = Classifier(sklEstimator=RandomForestClassifier(oob_score=True)).fit(sample=enmapClassificationSample)
        LoggerFlowObject(filename=r'c:\test\logRandomForestClassifier.txt').setSklEstimatorItems(estimator=estimator.sklEstimator()).logItems()
...
class TestRaster(TestCase):
    ...
    def test_subsetWavebands(self):
        ...
        raster = enmap.subsetWavebands(wavelength=hires.metadataWavelength(), filename=join(outdir, 'Raster.subsetWavebands.raster.bsq'))
        print(raster)
...
class TestRegressor(TestCase):
    ...
    def test_RegressorPredictOnSpectrallyHigherResolved(self):
        ...
        print(rfr.predict(filename=join(outdir, 'RegressorPredictOnSpectrallyHigherResolved.rfrRegression.bsq'), raster=enmap))
...
class TestRecipe(TestCase):
    ...
    def test_mergeSamples(self):
        ...
        fractionSample1 = enmapClassificationSample.synthMix(
            ...
            mixingComplexities={2: 0.5, 3: 0.5}, classLikelihoods='equalized',
            n=10, target=1)
        ...
        raster = Raster.fromArray(array=features, filename='raster.bsq',
                                  ...
                                  descriptions=[band.description() for band in fractionSample1.raster().dataset().bands()])  # a bit ugly, need to introduce Raster.descriptions() to get all band names directly!!!
        fraction = Fraction.fromArray(array=labels, filename='classification.bsq',
                                      noDataValues=fractionSample1.fraction().noDataValues(),
                                      descriptions=[band.description() for band in fractionSample1.fraction().dataset().bands()])  # a bit ugly, need to introduce Raster.descriptions() to get all band names directly!!!
