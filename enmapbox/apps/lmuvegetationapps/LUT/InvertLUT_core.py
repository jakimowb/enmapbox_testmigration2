...
from lmuvegetationapps.Resources.Spec2Sensor.Sensor_Info import get_wl
...
class RTM_Inversion:
    ...
    def sensor_setup(self, sensor):
        ...
        if sensor == 1:  # ASD
            ...
            self.exclude_bands_model = list(range(self.npara)) + [((i - self.offset)) + self.npara for i in self.exclude_bands[self.offset:]]
        elif sensor == 2:  # EnMAP
            # self.exclude_bands = range(78, 88) + range(128, 138) + range(161, 189) # Überlappung VNIR, Water1, Water2
            self.exclude_bands_model = list(range(self.npara)) + [i + self.npara for i in self.exclude_bands]
    ...
    def run_inversion(self, prg_widget=None, QGis_app=None):
        ...
        self.out_matrix = np.empty(shape=(self.npara, self.nrows, self.ncols))
        ...
        for i_ilut, ilut in enumerate(self.whichLUT_unique):
            ...
            LUT_params = lut[:self.npara, :]  # extract parameters
            lut = np.delete(lut, self.exclude_bands_model, axis=0)  # delete exclude_bands_model - members
            lut = self.add_noise(Ref_array=lut, type=self.noisetype, sigma=self.noiselevel)
            ...
            for isample in range(samples_size):
                ...
                if isample % 1000 == 0:
                    if prg_widget:
                        ...
                        prg_widget.gui.lblCaption_r.setText('Inverting pixels {:d}-{:d} of {:d}'.format(pix_current, pix_limit, npixel_valid))
