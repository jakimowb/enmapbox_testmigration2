...
class ProcessorPrediction:
    ...
    def predict_from_dump(self, prg_widget=None, qgis_app=None):
        ...
        nrows, ncols, nbands, self.grid, in_matrix = self.m.func._read_image(image=self.img_in, dtype=np.int)
        ...
        self.out_matrix = np.full(shape=(len(self.paras), nrows, ncols), fill_value=self.nodat[2], dtype=np.float)
