...
from lmuvegetationapps.SAILdata_v import *
import numpy as np
...
class Sail:
    ...
    def Pro4sail(self, rho, tau, LIDF, TypeLIDF, LAI, hspot, psoil, soil, understory=None, skyl=None, inform_trans=None):
        ...
        i19 = np.arange(19)
        x2 = -np.log(1.0 - (i19+1) * fint) / alf # vector
        x2[:,18] = np.ones(x2.shape[0]) # last element in x2 is 1.0
        y2 = -(ko+ks)*LAI*x2 + fhot*(1.0-np.exp(-alf*x2))/alf
        f2 = np.exp(y2)

        x1 = np.pad(x2, ((0, 0), (1, 0)), mode='constant')[:, :-1] # Shifts array by one and fills with constant = 0 (def)
        y1 = np.pad(y2, ((0, 0), (1, 0)), mode='constant')[:, :-1] # Shifts array by one and fills with constant = 0 (def)
        f1 = np.pad(f2, ((0, 0), (1, 0)), mode='constant', constant_values=1)[:, :-1] # Shifts array by one and fills with constant = 1
    ...
    def campbell(self, ALIA): # Calculates the Leaf Angle Distribution Function value (freq) Ellipsoidal distribution function from ALIA
        ...
        freq[excent < 1.0, :] = np.abs(dumm - (x2 * almx2 + alpha2 * np.arcsin(x2 / alpha[:,np.newaxis])))[excent < 1.0, :]
    ...
    def volscatt(self, ttl):
        ...
        bto = np.where(np.abs(cosbto) < 1, np.arccos(cosbto), np.where(self.tto[:,np.newaxis] < np.pi*0.5, np.pi, 0.0))
        ...
        bt2 = np.where(self.psi[:,np.newaxis] < btran1, btran1, np.where(self.psi[:,np.newaxis] <= btran2, self.psi[:,np.newaxis], btran2))
        bt3 = np.where(self.psi[:,np.newaxis] < btran1, btran2, np.where(self.psi[:,np.newaxis] <= btran2, btran2, self.psi[:,np.newaxis]))
