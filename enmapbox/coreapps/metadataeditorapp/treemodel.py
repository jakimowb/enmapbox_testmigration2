...
from enmapbox.coreapps.metadataeditorapp.treeitem import TreeItem
...
class TreeModel(QtCore.QAbstractItemModel):
    ...
    def data(self, index, role):
        ...
        #todo setData(index, role....editrole...
    ...
    def flags(self, index):
        self.cNAME = 'Name'
        self.cVALUE = 'Value'
        self.columnNames = [self.cNAME, self.cVALUE]
        if index.isValid():
            #print(index.parent().data(Qt.DisplayRole))
            columnName = self.columnNames[index.column()]
            if(index.parent().data(Qt.DisplayRole) == "Free Metadata by Domain (right click to rename and remove)"):
                flags = Qt.ItemIsEnabled | Qt.ItemIsSelectable | Qt.ItemIsEditable
            else:
                flags = Qt.ItemIsEnabled | Qt.ItemIsSelectable
            #print(index.parent())
            #if columnName in self.columnNames:  # allow check state
    ...
    def populateTree(self, obj, obj_name='', inspected_node_is_visible=None):
       ...
       # else:
        ...
        for item in range(0, len(obj)):
            try:
                level1Child = TreeItem(obj[item], obj[item].getName(), obj_name, is_attribute=None)
            except AttributeError:
                #if(obj[item]['name'] != 'Free Metadata by Domain (right click to rename and remove)'):
                level1Child = TreeItem(obj[item], obj[item]['name'], obj_name, is_attribute=None)

            self._root_item.append_child(level1Child)

            try:
                hasChildren = obj[item].hasChildren()
            except:
                hasChildren = ('children' in obj[item])

            if(hasChildren):
                try:
                    children = obj[item]['children']
                except:
                    children = obj[item].getChildren()
                ...
                for child in range(0, len(children)):
                    ...
                    try:
                        #print(children[child].getName())
                        level2Child = TreeItem(children[child], obj[item]['children'][child].getName(), obj_name, is_attribute=None)
                    except:
                        level2Child = TreeItem(children[child], obj_name, obj_name, is_attribute=None)

                    level1Child.append_child(level2Child)

                    #self.setHeaderData(0,Qt.Horizontal, None)

                    if ('value' in children[child]):
                        True
                    else:
                        #print(str(children[child]))

                        try:
                            length = len(children[child])
                        except:
                            #print("children[child]")
                            #print(children[child])
                            length = children[child].getLength()

                        for c in range(0, len(children[child].getContent())):
                            #print("level3")
                            #print(children[child].getContent()[c])

                            try:
                                #print("check3")
                                level3Child = TreeItem(children[child].getContent()[c], obj[item]['children'][child].getContent()[c].getName(), obj_name, is_attribute=None, hasChildren=False)
                            except:
                                level3Child = TreeItem(children[child].getContent()[c], obj_name, obj_name, is_attribute=True, hasChildren=False)

                            level2Child.append_child(level3Child)
