...
if __name__ == '__main__':
    ...
    lyr3 = QgsVectorLayer(landcover)

    QgsProject.instance().addMapLayers([lyr1, lyr2, lyr3])


    def activateMapTool(mapTool):
        assert isinstance(mapTool, QgsMapTool)
        mapTool.canvas().setMapTool(mapTool)

    def addMenu(canvas):
        assert isinstance(canvas, QgsMapCanvas)
        m = QMenu()
        a = m.addAction('Zoom Full')
        a.triggered.connec(lambda : activateMapTool(FullExtentMapTool(canvas)))

        a = m.addAction('CursorLocationMapTool')
        a.triggered.connec(lambda: activateMapTool(CursorLocationMapTool(canvas)))

        a = m.addAction('PixelScale')
        a.triggered.connec(lambda: activateMapTool(PixelScaleExtentMapTool(canvas)))

        a = m.addAction('PixelScale')
        a.triggered.connec(lambda: activateMapTool(PixelScaleExtentMapTool(canvas)))

        a = m.addAction('Point Layers Map Tool')
        a.triggered.connec(lambda: activateMapTool(PointLayersMapTool(canvas)))

    w = QWidget()
    l = QHBoxLayout()
    canvas1 = QgsMapCanvas()
    canvas1.setWindowTitle('Canvas1')
    canvas1.setLayers([lyr1])
