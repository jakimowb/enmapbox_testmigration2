...
class ConvexHullAlgorithm(EnMAPProcessingAlgorithm):
    ...
    def processAlgorithm(
        ...
        with open(filenameLog + '.log', 'w') as logfile:
            ...
            if filenameConvexHull is not None:
                writerConvexHull = Driver(
                    filenameConvexHull, self.GTiffFormat, self.DefaultGTiffCreationOptions, feedback
                ).createLike(reader)
            if filenameContinuumRemoved is not None:
                writerContinuumRemoved = Driver(
                    filenameContinuumRemoved, self.GTiffFormat, self.DefaultGTiffCreationOptions, feedback
