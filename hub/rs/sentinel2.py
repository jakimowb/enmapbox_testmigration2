__author__ = 'janzandr'
import glob, os, hub.file, hub.gdal.util, hub.envi, numpy
...
#    resolutions = numpy.array([elem.text for elem in root.iter('RESOLUTION')])
    ...
    for bname in bnames2:
        ...
        jp2files = hub.file.filesearch(os.path.join(infolder, 'GRANULE'), '*'+bname+'.jp2')
        ...
        for jp2file, warpedjp2file in zip(jp2files,warpedjp2files):
            hub.gdal.util.gdalwarp(warpedjp2file, jp2file, '-of VRT -overwrite -wm 2000 -s_srs epsg:'+str(epsg))

        outfileSingle = outfile+'_'+bname+'.vrt'

        hub.gdal.util.mosaic(outfileSingle, warpedjp2files)
#        hub.envi.vrt2mos(outfileSingle)
    ...
    for targetResolution in [10,20]: #[10,20,60]:
        ...
        hub.gdal.util.gdal_translate(outenvi, outvrt, '-of ENVI')
