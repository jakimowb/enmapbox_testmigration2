...
a state-of-the-art Integrated Development Environment (IDE) like PyCharm_. It offers run-time debugging,
...
3. Create a conda environment with QGIS
=======================================

The following setup bases on `this description <http://planet.qgis.org/planet/user/22/tag/conda/>`_ and
installs QGIS into a `miniconda <https://docs.conda.io/en/latest/miniconda.html>`_ environment.
...
3. `install <https://docs.conda.io/projects/conda/en/latest/user-guide/tasks/manage-pkgs.html#installing-packages>`_
   QGIS from the `conda-forge <https://conda-forge.org/>`_ channel

    .. code-block:: batch

            (qgis_stable) $>conda install qgis --channel=conda-forge

4. install other packages (they are required for some QGIS plugins like the EnMAP-Box)

    .. code-block:: batch

         (qgis_stable) $>conda install pandas --channel=conda-forge
         (qgis_stable) $>pip install pb-tool
...
5. Now you can start QGIS, the Qt Designer and the Qt Assistant from your conda shell:
...
6. Start python and check it's version. It should be a Python >= 3.7. Call `quit()` to return
   to the conda shell:
...
4. Switch to *Project Structure* and add
   ...
   This makes PyQGIS API available to your Python.

    .. admonition:: Note

        This way you can easily include other directories to the *PYTHONPATH* of your project, e.g. code from other repositories.
...
5. (Windows + macOS) set the QGIS_PREFIX_PATH
   ...
   This way PyCharm will run python files in a environment that has `QGIS_PREFIX_PATH` set.

    .. figure:: img/pycharm_QGIS_PREFIX_PATH.png


    You might also like to use the conda environment in your Pycharm terminal, which provided access to your
    local shell. Open *Tools > Terminal* and the shell path to `cmd.exe "/K" <your conda installation>\Scripts\activate.bat qgis_stable`
...
7. Test the Python environment
    ...
    Open Python Console and start a `QgsApplication <https://qgis.org/api/classQgsApplication.html>`_ instance.
    ...
    .. code-block:: bash
        ...
        Plugin Path:		D:/miniconda3/envs/qgis_stable/plugins
        Package Data Path:	D:/miniconda3/envs/qgis_stable/.
        Active Theme Name:
        Active Theme Path:	D:/miniconda3/envs/qgis_stable/./resources/themes\\icons/
        Default Theme Path:	:/images/themes/default/
        SVG Search Paths:	D:/miniconda3/envs/qgis_stable/./svg/
                C:\Users\<user>\AppData\Local\Temp\QGIS-PythonTestConfigPath6_5qiyqh\profiles\default/svg/
        User DB Path:	D:/miniconda3/envs/qgis_stable/./resources/qgis.db
        Auth DB Path:	C:\Users\geo_beja\AppData\Local\Temp\QGIS-PythonTestConfigPath6_5qiyqh\profiles\default/qgis-auth.db
        <qgis._core.QgsApplication object at 0x0000017A8B4029D0>

    To start the EnMAP-Box, call:
    ...
    Now open a PyCharm Terminal window. If the terminal environment was setup well, you
    can start the EnMAP-Box from the shell with:
