...
This is useful for sensor product imported with the EnMAP-Box, which dumps all metadata to this location::
    ...
    # Sentinel-2
    startTime = parseDateTimeString(gdalDataset.GetMetadataItem('PRODUCT_START_TIME'))
    endTime = parseDateTimeString(gdalDataset.GetMetadataItem('PRODUCT_STOP_TIME'))

    # EnMAP and DESIS
    startTime = parseDateTimeString(gdalDataset.GetMetadataItem('startTime'))
    endTime = parseDateTimeString(gdalDataset.GetMetadataItem('endTime'))

    # PRISMA
    startTime = parseDateTimeString(gdalDataset.GetMetadataItem('Product_StartTime'))
    endTime = parseDateTimeString(gdalDataset.GetMetadataItem('Product_StopTime'))

    # Google Earth Engine
    startMSecs = gdalDataset.GetMetadataItem('system:time_start')
    endMSecs = gdalDataset.GetMetadataItem('system:time_end')
