...
class ClassifierPredict(EnMAPAlgorithm):
    ...
    def cookbookRecipes(self):
        return ['classification.html#predict-classification']
...
'''.format(akey, '*' * len(akey), akey, '*' * len(akey))
            ...
            if len(alg.cookbookRecipes()) > 0:
                text += 'Used in the Cookbook Recipes\n'
                for recipeUrlTail in alg.cookbookRecipes():
                    recipeName = EnMAPAlgorithm.COOKBOOK_NAME[recipeUrlTail.split('#')[0]]
                    recipeUrl = '{}/{}'.format(EnMAPAlgorithm.COOKBOOK_URL, recipeUrlTail)
                    text += '    - `{} <{}>`_ (section: {})\n\n'.format(recipeName, recipeUrl, recipeUrlTail.split('#')[1])
