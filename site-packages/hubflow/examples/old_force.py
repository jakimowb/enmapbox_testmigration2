...
class ForceL2(ForceLevel):
    ...
    def mean(self, basename, levelName, dateRangeFilter, sensorFilter, folder=None):
        if folder is None:
            folder = self.db().folder()

        for tileName in self.db().spatialFilter().tileNames():
            tile = self.tile(name=tileName)
            assert isinstance(tile, ForceL2Tile)
            tile.mean(filename=join(folder, levelName, tile.name(), basename),
                      dateRangeFilter=dateRangeFilter, sensorFilter=sensorFilter)

    def std(self, basename, levelName, dateRangeFilter, sensorFilter, folder=None):
        if folder is None:
            folder = self.db().folder()
        ...
        for tileName in self.db().spatialFilter().tileNames():
            tile = self.tile(name=tileName)
            assert isinstance(tile, ForceL2Tile)
            tile.std(filename=join(folder, levelName, tile.name(), basename),
                     dateRangeFilter=dateRangeFilter, sensorFilter=sensorFilter)
